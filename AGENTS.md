...
- [How a tag-triggered workflow run publishes mdsmith to npm, PyPI, the Visual Studio Marketplace, Open VSX, and GitHub Releases — the workflow structure, the OIDC trusted publishers it relies on, the `release` environment that gates every publishing job, the separate website deploy, and the supply-chain hardening features baked into the pipeline.](docs/development/release.md)
...
- [A single static Go binary with no runtime to boot. The workspace walk runs in parallel, embeds are linted once, and `check` is built for the hot path so CI and editor feedback stay instant.](docs/features/performance.md)
- [The README build, Go Report Card, and coverage badges report live project health. mdsmith lints its own docs with the rules it ships, and a coverage gate blocks merges that drop below the line.](docs/features/quality.md)
