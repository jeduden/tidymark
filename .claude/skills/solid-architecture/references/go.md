---
title: Go SOLID and clean architecture
summary: >-
  Go-specific patterns for SOLID and clean
  architecture inside mdsmith's cmd/ and
  internal/ packages. Pair with the main
  SKILL.md for triggers and modes.
---
# SOLID and clean architecture in Go

Go-specific guidance for mdsmith's Go code.
The main `SKILL.md` covers when to invoke
this skill; this file covers how to apply
each principle in Go.
...
map (see `SKILL.md` §"Project layering").
...
- Use `testify/require` for preconditions
  that should abort the test; `testify/assert`
  for soft checks.
