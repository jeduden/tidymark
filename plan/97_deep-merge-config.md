...
4. [x] Verify `mdsmith check .` against this repo
   produces the same diagnostics under the new merge
   (plan 95's `mdsmith config show` will surface
   per-leaf provenance once landed; the
   `EffectiveWithProvenance` API powers it).
5. [x] Document the change in
   `docs/development/index.md` and add a "merge
   semantics" subsection to `docs/reference/cli.md`.
6. [x] Update plan 92's Conflict resolution section
   to point at this plan as the live behavior.
...
- [x] `EffectiveWithProvenance` returns per-leaf
      provenance — each leaf setting carries the
      layer that contributed its value (covered by
      test). Plan 95 will wire this into
      `mdsmith config show`.
- [x] No existing rule's behavior changes
      unexpectedly: a regression test runs a config
      that fully restates a rule at every layer and
      asserts the result matches block-replace
      semantics on scalar leaves
      (`TestEffectiveBlockReplaceRegression`); the
      pre-existing kind tests in `kinds_test.go`
      still pass against the new merge.
