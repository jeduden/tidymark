...
- [x] Add a per-leaf provenance tracker to the config-
  merge pipeline. Each leaf setting's final value
  carries a list `[{layer, value, source}]`.
- [x] Add `mdsmith kinds list` (text + `--json`).
- [x] Add `mdsmith kinds show <name>` and
  `mdsmith kinds path <name>`.
- [x] Add `mdsmith kinds resolve <file>` rendering the
  resolved kind list and per-leaf provenance
  summary; add `--json`.
- [x] Add `mdsmith kinds why <file> <rule>` rendering
  the full merge chain for a single rule; add
  `--json`.
- [x] Add `--explain` flag to `check` and `fix`. After
...
- [x] Document the JSON schema briefly in
  `docs/reference/cli.md`.
- [x] `mdsmith help kinds-cli` prints the subcommand
  summary.
