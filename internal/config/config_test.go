...
// TestLoad_ArchetypesKeyReturnsError ensures the removed `archetypes:` config
// key produces a clear error directing users to `kinds:`.
func TestLoad_ArchetypesKeyReturnsError(t *testing.T) {
	dir := t.TempDir()
	cfgPath := filepath.Join(dir, ".mdsmith.yml")
	require.NoError(t, os.WriteFile(cfgPath, []byte("archetypes:\n  roots:\n    - archetypes\n"), 0o644))
	_, err := Load(cfgPath)
	require.Error(t, err)
