...
func (r *Rule) ApplySettings(settings map[string]any) error {
	for k, v := range settings {
		...
		case "schema":
			...
			// Reject bare names (no path separator or extension). A name
			// like "story" is not a valid path and would have previously
			// triggered an archetype lookup that no longer exists.
			hasSlash := strings.Contains(s, "/") || strings.Contains(s, string(filepath.Separator))
			if s != "" && !hasSlash && !strings.HasSuffix(s, ".md") {
				return fmt.Errorf(
					"required-structure: schema %q looks like a name, not a path; "+
						"use an explicit file path (e.g. schemas/%s.md)", s, s)
			}
			r.Schema = s
		case "archetype":
			return fmt.Errorf(
				"required-structure: setting \"archetype\" is no longer supported; " +
					"use \"schema\" with an explicit file path instead")
		case "archetype-roots":
			return fmt.Errorf(
				"required-structure: setting \"archetype-roots\" is no longer supported; " +
					"use \"schema\" with an explicit file path instead")
		case "placeholders":
...
// Files identified this way are treated as schema sources: <?require?>
// directives in them are meaningful, and the rule skips structural
