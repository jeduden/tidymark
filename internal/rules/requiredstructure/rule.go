...
func (r *Rule) ApplySettings(settings map[string]any) error {
	for k, v := range settings {
		...
		case "archetype", "archetype-roots":
			return fmt.Errorf(
				"required-structure: %q is no longer supported; "+
					"use 'schema:' with an explicit path, or declare a kind "+
					"with 'rules.required-structure.schema:' set", k)
		case "placeholders":
...
// Schema files are treated as sources: <?require?> directives in them
// are meaningful, and the rule skips structural validation against them.
