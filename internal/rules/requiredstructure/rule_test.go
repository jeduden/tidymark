...
func TestDefaultSettings(t *testing.T) {
	...
	// archetype and archetype-roots are no longer settings
	_, hasArchetype := ds["archetype"]
	assert.False(t, hasArchetype, "archetype must not be a default setting")
	_, hasArchetypeRoots := ds["archetype-roots"]
	assert.False(t, hasArchetypeRoots, "archetype-roots must not be a default setting")
}

// archetype and archetype-roots settings produce errors directing users to schema:.

func TestApplySettings_ArchetypeProducesError(t *testing.T) {
	r := &Rule{}
...
func TestApplySettings_ArchetypeRootsProducesError(t *testing.T) {
	r := &Rule{}
...
// TestCheck_SchemaFileSkippedAsSchemaSource verifies that the schema file
// itself is skipped during structural validation.
func TestCheck_SchemaFileSkippedAsSchemaSource(t *testing.T) {
	schPath := writeSchema(t, "# ?\n\n## Goal\n")
	r := &Rule{Schema: schPath}
	// Lint the schema file itself — it must not produce diagnostics.
	f := newTestFile(t, schPath, "# ?\n\n## Goal\n")
	diags := r.Check(f)
	expectDiags(t, diags, 0)
}

// TestCheck_RequireWarningOnlyInNonSchemaFiles verifies that <?require?>
// produces a warning only in non-schema files.
func TestCheck_RequireWarningOnlyInNonSchemaFiles(t *testing.T) {
	schPath := writeSchema(t, "<?require\nfilename: \"doc-*.md\"\n?>\n# ?\n")
	r := &Rule{Schema: schPath}
	// Lint a normal file with <?require?> — should warn.
	f := newTestFile(t, "doc.md", "<?require\nfilename: \"doc-*.md\"\n?>\n# Title\n")
	diags := r.Check(f)
...
// isSchemaFile: file not matching schema path returns false
func TestIsSchemaFile_NonMatchingFile(t *testing.T) {
	schPath := writeSchema(t, "# ?\n")
	r := &Rule{Schema: schPath}
	// A different file must not be identified as the schema.
	f := &lint.File{Path: "some-other.md"}
	result := r.isSchemaFile(f)
	assert.False(t, result)
}

// isSchemaFile: when schema is empty, always false
func TestIsSchemaFile_EmptySchema(t *testing.T) {
	r := &Rule{}
