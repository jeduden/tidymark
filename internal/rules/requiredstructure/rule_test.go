...
func TestDefaultSettings(t *testing.T) {
	...
	assert.False(t, hasArchetype, "DefaultSettings must not include 'archetype' key")
	_, hasArchetypeRoots := ds["archetype-roots"]
	assert.False(t, hasArchetypeRoots, "DefaultSettings must not include 'archetype-roots' key")
}

func TestApplySettings_ArchetypeReturnsError(t *testing.T) {
	r := &Rule{}
...
func TestApplySettings_ArchetypeRootsReturnsError(t *testing.T) {
	r := &Rule{}
...
func TestCheck_RequireDirectiveWarnsInNonSchemaFile(t *testing.T) {
	// A file with <?require?> that is not the configured schema file
	// should get a warning.
	src := "<?require\nfilename: \"doc-*.md\"\n?>\n# Title\n"
	r := &Rule{} // no schema configured
...
// isSchemaFile: non-matching path returns false
func TestIsSchemaFile_NonMatchingPath(t *testing.T) {
