...
func TestDefaultSettings(t *testing.T) {
	...
	// archetype and archetype-roots are no longer part of DefaultSettings.
	if _, ok := ds["archetype"]; ok {
		t.Error("DefaultSettings must not include 'archetype' key")
	}
	if _, ok := ds["archetype-roots"]; ok {
		t.Error("DefaultSettings must not include 'archetype-roots' key")
	}
}
...
func TestApplySettings_ArchetypeRootsSettingReturnsError(t *testing.T) {
	r := &Rule{}
	err := r.ApplySettings(map[string]any{"archetype-roots": []any{"tmpl"}})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no longer supported")
}

func TestApplySettings_SchemaNameWithoutPathReturnsError(t *testing.T) {
	r := &Rule{}
	err := r.ApplySettings(map[string]any{"schema": "story"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "looks like a name, not a path")
}

func TestApplySettings_SchemaWithPathSeparatorIsAccepted(t *testing.T) {
	r := &Rule{}
...
func TestApplySettings_SchemaWithMdExtensionIsAccepted(t *testing.T) {
	r := &Rule{}
...
func TestCheck_RequireWarningInNonSchemaFile(t *testing.T) {
	// <?require?> in a file with no schema configured produces a warning.
...
func TestCheck_RequireWarningSuppressedForSchemaFile(t *testing.T) {
	// <?require?> in the configured schema file itself must not warn.
	root := t.TempDir()
	schemaPath := filepath.Join(root, "schema.md")
	require.NoError(t, os.WriteFile(schemaPath,
		[]byte("<?require\nfilename: \"story-*.md\"\n?>\n# ?\n"), 0o644))
	f := newFileInRoot(t, root, schemaPath,
		"<?require\nfilename: \"story-*.md\"\n?>\n# ?\n")
...
// isSchemaFile: file not matching schema path → false
func TestIsSchemaFile_NonMatchingFile(t *testing.T) {
	r := &Rule{Schema: "schemas/story.md"}
	f := &lint.File{Path: "docs/other.md"}
	assert.False(t, r.isSchemaFile(f))
}

func TestIsSchemaFile_MatchingFile(t *testing.T) {
	root := t.TempDir()
	schemaPath := filepath.Join(root, "schema.md")
	require.NoError(t, os.WriteFile(schemaPath, []byte("# ?\n"), 0o644))
	r := &Rule{Schema: schemaPath}
	f := &lint.File{Path: schemaPath}
	assert.True(t, r.isSchemaFile(f))
}

func TestIsSchemaFile_NoSchema(t *testing.T) {
	r := &Rule{}
	f := &lint.File{Path: "any.md"}
	assert.False(t, r.isSchemaFile(f))
}
