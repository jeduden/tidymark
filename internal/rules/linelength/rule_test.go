...
func TestCheck_CustomMax120(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for 100-char line with max=120, got %d", len(diags))
}
...
func TestCheck_ExcludeCodeBlocks_FencedCodeBlockSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for long line inside fenced code block, got %d", len(diags))
}
...
func TestCheck_ExcludeCodeBlocks_IndentedCodeBlockSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for long line inside indented code block, got %d", len(diags))
}
...
func TestCheck_ExcludeURLs_URLOnlyLineSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for URL-only long line, got %d", len(diags))
}
...
func TestCheck_ExcludeURLs_URLOnlyWithLeadingWhitespace(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for URL-only line with whitespace, got %d", len(diags))
}
...
func TestCheck_ExcludeURLs_URLWithTextNotSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for line with URL plus text, got %d", len(diags))
}
...
func TestCheck_EmptyExclude_URLOnlyLineFlagged(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: []string{}}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for URL-only line (exclude=[]), got %d", len(diags))
}
...
func TestCheck_EmptyFile(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for empty file, got %d", len(diags))
}
...
func TestCheck_FencedCodeBlockFenceLinesAlsoSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for long fence opening line, got %d", len(diags))
}
...
func TestCheck_HttpURLSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for http URL-only long line, got %d", len(diags))
}
...
func TestCheck_MixedContentWithCodeBlockAndLongLines(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic (only the line outside code block), got %d", len(diags))
	if diags[0].Line != 1 {
...
func TestCheck_TildeFencedCodeBlockSkipped(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for long line inside tilde fenced code block, got %d", len(diags))
}
...
func TestCheck_EmptyFencedCodeBlock(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for empty fenced code block, got %d", len(diags))
}
...
func TestApplySettings_EmptyExclude(t *testing.T) {
	...
	require.NoError(t, err, "unexpected error: %v", err)
	assert.Len(t, r.Exclude, 0, "expected empty Exclude, got %v", r.Exclude)
}

func TestApplySettings_InvalidExcludeValue(t *testing.T) {
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	err := r.ApplySettings(map[string]any{"exclude": []any{"invalid"}})
	require.Error(t, err, "expected error for invalid exclude value")
}
...
func TestApplySettings_StrictTrueDeprecationShim(t *testing.T) {
	...
	require.NoError(t, err, "unexpected error: %v", err)
	assert.Len(t, r.Exclude, 0, "expected empty Exclude for strict:true, got %v", r.Exclude)
}
...
func TestApplySettings_StrictFalseDeprecationShim(t *testing.T) {
	...
	require.NoError(t, err, "unexpected error: %v", err)
	assert.Len(t, r.Exclude, 3, "expected 3 exclude items for strict:false, got %v", r.Exclude)
}
...
func TestDefaultSettings(t *testing.T) {
	...
	require.True(t, ok, "expected exclude to be []string, got %T", ds["exclude"])
...
func TestCheck_ExcludeTables_LongTableRowSkipped(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for long table row with tables excluded, got %d", len(diags))
}
...
func TestCheck_NoExcludeTables_LongTableRowFlagged(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for long table row without tables excluded, got %d", len(diags))
}
...
func TestCheck_ExcludeOnlyCodeBlocks(t *testing.T) {
	...
	require.Len(t, diags, 2, "expected 2 diagnostics (URL + table), got %d", len(diags))
}
...
func TestCheck_HeadingMax_HeadingWithinHeadingMaxButOverMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for heading within heading-max, got %d", len(diags))
}
...
func TestCheck_HeadingMax_HeadingOverHeadingMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for heading over heading-max, got %d", len(diags))
...
func TestCheck_HeadingMax_UnsetInheritsFromMax(t *testing.T) {
	...
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for heading over max (heading-max unset), got %d", len(diags))
}
...
func TestCheck_HeadingMax_NonHeadingUsesBaseMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for non-heading line over max, got %d", len(diags))
...
func TestCheck_HeadingMax_Level2Heading(t *testing.T) {
	...
	r := &Rule{Max: 60, HeadingMax: intPtr(80), Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for level-2 heading within heading-max, got %d", len(diags))
}
...
func TestCheck_HeadingMax_SetextHeading(t *testing.T) {
	...
	r := &Rule{Max: 80, HeadingMax: intPtr(120), Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for Setext heading within heading-max, got %d", len(diags))
}
...
func TestCheck_CodeBlockMax_CodeLineWithinCodeBlockMaxButOverMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for code line within code-block-max, got %d", len(diags))
}
...
func TestCheck_CodeBlockMax_CodeLineOverCodeBlockMax(t *testing.T) {
	...
	r := &Rule{Max: 80, CodeBlockMax: intPtr(120), Exclude: []string{"tables", "urls"}}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for code line over code-block-max, got %d", len(diags))
...
func TestCheck_CodeBlockMax_UnsetInheritsFromMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for code line over max (code-block-max unset), got %d", len(diags))
}
...
func TestCheck_CodeBlockMax_ExcludeStillSkips(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for code line when code-blocks excluded, got %d", len(diags))
}
...
func TestCheck_CodeBlockMax_NonCodeLineUsesBaseMax(t *testing.T) {
	...
	r := &Rule{Max: 80, CodeBlockMax: intPtr(120), Exclude: []string{"tables", "urls"}}
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for non-code line over max, got %d", len(diags))
	expected := "line too long (90 > 80)"
	if diags[0].Message != expected {
		t.Errorf("expected message %q, got %q", expected, diags[0].Message)
	}
...
func TestCheck_Stern_LongLineWithSpacesPastLimit(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for line with spaces past limit in stern mode, got %d", len(diags))
}
...
func TestCheck_Stern_DisabledStillFlags(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic with stern disabled, got %d", len(diags))
}
...
func TestCheck_Stern_ExcludeInteraction(t *testing.T) {
	...
	r := &Rule{Max: 80, Stern: true, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for excluded code line in stern mode, got %d", len(diags))
}
...
func TestCheck_Stern_WithHeadingMaxNoSpacePast(t *testing.T) {
	...
	r := &Rule{Max: 80, HeadingMax: intPtr(60), Stern: true, Exclude: defaultExclude()}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics for heading without spaces past heading-max, got %d", len(diags))
}
...
func TestCheck_Stern_WithCodeBlockMax(t *testing.T) {
	...
	diags := r.Check(f)
	require.Len(t, diags, 1, "expected 1 diagnostic for code line with space past code-block-max, got %d", len(diags))
}
...
func TestApplySettings_InvalidHeadingMaxType(t *testing.T) {
	...
	require.Error(t, err, "expected error for non-int heading-max")
}

func TestApplySettings_HeadingMaxMustBePositive(t *testing.T) {
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	err := r.ApplySettings(map[string]any{"heading-max": 0})
	require.Error(t, err, "expected error for zero heading-max")
}
...
func TestApplySettings_InvalidCodeBlockMaxType(t *testing.T) {
	...
	require.Error(t, err, "expected error for non-int code-block-max")
}

func TestApplySettings_CodeBlockMaxMustBePositive(t *testing.T) {
	r := &Rule{Max: 80, Exclude: defaultExclude()}
	err := r.ApplySettings(map[string]any{"code-block-max": -5})
	require.Error(t, err, "expected error for negative code-block-max")
}
