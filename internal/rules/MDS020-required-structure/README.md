...
|----------------|--------|---------|----------------------------------------------------------------------------------------------------------------------------|
| `schema`       | string | `""`    | Path to a schema file (must contain a `/` or end in `.md`)                                                                 |
...
`<?require?>` directives. Use overrides or
[kinds](../../../docs/guides/directives/enforcing-structure.md)
to apply schemas to specific file groups.

`schema` must be an explicit file path. A bare name
like `schema: story` produces a clear error directing
users to supply the full path (e.g. `schemas/story.md`).
...
Apply a schema to all story files using a kind:
