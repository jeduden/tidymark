...
func TestCheck_StartOfFile(t *testing.T) {
	...
	r := &Rule{}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics (start of file), got %d: %v", len(diags), diags)
}
...
func TestCheck_EndOfFile(t *testing.T) {
	...
	r := &Rule{}
	diags := r.Check(f)
	require.Len(t, diags, 0, "expected 0 diagnostics (end of file), got %d: %v", len(diags), diags)
}
