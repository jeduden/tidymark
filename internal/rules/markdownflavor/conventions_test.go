...
func TestLookup_UserConvention_BuiltInStillFound(t *testing.T) {
	user := map[string]Convention{
		"our-team": {Name: "our-team"},
	}
...
func TestLookup_UserConvention_NilUserMap(t *testing.T) {
	// nil user map falls through to built-ins
	c, err := Lookup("portable", nil)
	require.NoError(t, err)
	assert.Equal(t, "portable", c.Name)
}

func TestLookup_UserConvention_NilUserMapUnknown(t *testing.T) {
	_, err := Lookup("bogus", nil)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "bogus")
}

func TestLookup_ReturnsDeepCopyOfUserConvention(t *testing.T) {
	user := map[string]Convention{
		"our-team": {
			Name:   "our-team",
			Flavor: FlavorGFM,
	...
	require.NoError(t, err)
	first.Rules["no-inline-html"].Settings["allow"] = []any{"tampered"}
	first.Rules["new-rule"] = RulePreset{Enabled: true}

	second, err := Lookup("our-team", user)
	require.NoError(t, err)
	assert.Equal(t, []any{"details"},
		second.Rules["no-inline-html"].Settings["allow"],
		"second Lookup must return original value")
	_, hasNew := second.Rules["new-rule"]
