...
`schema:` setting and apply it to files via
`kind-assignment:` or front-matter `kinds:`.
...
Use `mdsmith kinds list` to see all declared kinds
and their merged bodies. Use `mdsmith kinds show
<name>` to print one kind's body.
