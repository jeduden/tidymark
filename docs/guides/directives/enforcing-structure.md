...
## Reusable schemas via kinds

Instead of repeating `schema: path/to/schema.md` in
every override, declare a kind that bundles the
`required-structure` settings and assign files to it.
...
Use `mdsmith kinds list` to see all declared kinds and
`mdsmith kinds show <name>` to inspect one. The
`mdsmith kinds path <name>` subcommand prints the
resolved schema path for a kind's `required-structure`
rule, if any.
